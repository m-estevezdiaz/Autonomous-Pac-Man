...
SONAR_NOISE_PROBS = [2 ** (SONAR_MAX - abs(v)) /
                     SONAR_DENOMINATOR for v in SONAR_NOISE_VALUES]
...
def getNoisyDistance(pos1, pos2):
    ...
    # return max(0, distance + util.sample(SONAR_NOISE_PROBS,
    # SONAR_NOISE_VALUES))
    return distance
...
class GameState:
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        for i in range(1, self.getNumAgents()):
            self.ghostDirections[a] = (
                state.data.agentStates[i].configuration.getDirection())
            a += 1
    ...
    def initialize(self, layout, numGhostAgents=1000):
        ...
        # print("Posición inicial de Pacman:", self.getPacmanPosition())
