...
# pip install python-weka-wrapper==0.3.7
...
class Weka:
	...
	# Devuelve el cluster al que pertenece la instancia pasada como parametro
	# @param modelName: Nombre del fichero que contiene el modelo generado en weka
	# @param x: La instancia para la que se pretende determinar el cluster al que pertenece
	# @param arffName: El nombre del fichero arff que se ha utilizado para generar el modelo en Weka
	# @return cl: El cluster al que pertenece la instancia
	#
	def cluster(self, modelName, x, arffName, debug=False):
		# Carga el arrf para conocer la estructura de las instancias
		loader = Loader(classname="weka.core.converters.ArffLoader")
		data = loader.load_file(arffName)
		...
		# Carga del modelo generado en Weka
		objects = serialization.read_all(modelName)
		cls = Clusterer(jobject=objects[0])
		if(debug): 
			print("Loaded model...")
			print(cls)
		...
		# Clasifica la instancia dentro de un determinado cluster
		inst = Instance.create_instance(x) 
		inst.dataset = data
		cl = cls.cluster_instance(inst)  # 0-based cluster index
		dist = cls.distribution_for_instance(inst)   # cluster membership distribution
		if(debug): 
...
#print(weka.predict("./models/randomF-glass.model", x, "../weka-3-8-1/data/glass.arff", True))

#x = [1.51793,12.79,3.5,1.12,73.03,0.64,8.77,0,0]
#x = [1.51299,14.4,1.74,1.54,74.55,0,7.59,0,0]
#print(weka.predict("./models/j48-glass.model", x, "../weka-3-8-1/data/glass.arff", True))

#x = ['M',0.455,0.365,0.095,0.514,0.2245,0.101,0.15]
#print(weka.predict("./models/m5p-abalone.model", x, "../weka-3-8-1/data/abalone.arff", True))

#x = ['M',0.455,0.365,0.095,0.514,0.2245,0.101,0.15]
#print(weka.predict("./models/perceptron-abalone.model", x, "../weka-3-8-1/data/abalone.arff", True))

#x = ['M',0.455,0.365,0.095,0.514,0.2245,0.101,0.15]
#print(weka.predict("./models/kstar-abalone.model", x, "../weka-3-8-1/data/abalone.arff", True))

#x = ['sunny','mild','high','TRUE']
#print(weka.predict("./models/randomF-weather.nominal.3.8.6.model", x, "../weka-3-8-6/data/weather.nominal.arff", True))

#x = ['young','myope','no','normal','soft']
#print(weka.cluster("./models/lenses-kmeans.model", x, "../weka-3-8-1/data/contact-lenses.arff", True))

#x = [14.7,7.0,200.0,302.1,14.8,15.2,16.8]
#print(weka.cluster("./models/cpu-em.model", x, "../weka-3-8-1/data/cpu.arff", True))
